  ...
  return (
    <div style={{ padding: 40, textAlign: "center", color: "#3b82f6" }}>
      <div style={{
        ...
        border: "3px solid #2a3a4a", borderTopColor: "#3b82f6",
  ...
  return (
    <button
      ...
      style={{
        ...
        color: active ? activeColor : hovered ? activeColor : "#94a3b8",
        ...
        return (
          <button key={i} disabled={showResults}
            ...
            style={{
              ...
              background: showResults ? "transparent" : "#152030",
              border: `1px solid ${isMyVote ? "#3b82f6" : "#2a3a4a"}`,
              borderRadius: 8, padding: "10px 14px", marginBottom: 6,
              cursor: showResults ? "default" : "pointer",
              color: "#e2e8f0", fontSize: 14, fontFamily: "'Crimson Pro', serif",
...
function QuoteEmbed({ post }) {
  ...
  return (
    <div style={{
      marginTop: 10, border: "1px solid #2a3a4a", borderRadius: 12,
      padding: "10px 14px", background: "#1a2535", cursor: "pointer",
    }}>
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
        ...
        <span style={{ fontWeight: 700, fontSize: 13, color: "#e2e8f0" }}>{post.quote_user_name}</span>
        {post.quote_user_verified && <span style={{ color: "#3b82f6", fontSize: 12 }}>✓</span>}
        <span style={{ color: "#94a3b8", fontSize: 12 }}>@{post.quote_user_handle}</span>
        <span style={{ color: "#2a3a4a" }}>·</span>
        <span style={{ color: "#94a3b8", fontSize: 12 }}>{timeAgo(post.quote_created_at)}</span>
      </div>
      <p style={{ color: "#94a3b8", fontSize: 13, lineHeight: 1.5 }}>{post.quote_content}</p>
...
function PostCard({ post, onLike, onRepost, onBookmark, onDelete, onReply, onVote, onQuote, onClickPost, onClickUser, isOwn, currentUser }) {
  ...
  return (
    <div style={{
      borderBottom: "1px solid #2a3a4a", padding: "16px 20px",
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
              <span
                onClick={e => { e.stopPropagation(); onClickUser && onClickUser(p.user_handle); }}
                style={{ fontWeight: 700, color: "#e2e8f0", fontSize: 15, fontFamily: "'DM Serif Display', serif", cursor: "pointer" }}
              >
                {p.user_name || p.user}
              </span>
              {(p.user_verified || p.verified) && <span style={{ color: "#3b82f6" }}>✓</span>}
              <span style={{ color: "#94a3b8", fontSize: 14 }}>@{p.user_handle || p.handle?.replace("@","")}</span>
              <span style={{ color: "#2a3a4a" }}>·</span>
            ...
            <div style={{ position: "relative" }} ref={menuRef}>
              <button onClick={e => { e.stopPropagation(); setShowMenu(!showMenu); }}
                style={{ background: "none", border: "none", color: "#94a3b8", cursor: "pointer", fontSize: 18, padding: "2px 8px", borderRadius: "50%" }}
                  ...
                  {[
                    ...
                    <button key={i} onClick={e => { e.stopPropagation(); item.action(); setShowMenu(false); }}
                      style={{
                        ...
                        color: item.danger ? "#f91880" : "#e2e8f0",
          ...
          {p.image_url && (
            <img src={p.image_url} alt="" onClick={e => e.stopPropagation()} style={{
              ...
              maxHeight: 400, objectFit: "cover", border: "1px solid #2a3a4a",
            }} />
          )}
          ...
          <div style={{ display: "flex", gap: 4, marginTop: 12, marginLeft: -8 }}>
            ...
            <ActionBtn icon="👁️" count={p.views_count ?? p.views ?? 0} active={false} activeColor="#94a3b8"
          ...
          {/* Svar-boks */}
          {showReplyBox && (
            <div style={{ marginTop: 12, padding: 12, background: "#1a2535", borderRadius: 12, border: "1px solid #2a3a4a" }}
                ...
                <div style={{ flex: 1 }}>
                  <textarea value={replyText} onChange={e => setReplyText(e.target.value)}
                    ...
                    style={{
                      ...
                      color: "#e2e8f0", fontFamily: "'Crimson Pro', serif", fontSize: 14,
                      resize: "none", minHeight: 60,
                    }}
                  />
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <span style={{ color: "#94a3b8", fontSize: 12 }}>{280 - replyText.length}</span>
                    <button
                      ...
                      style={{
                        background: replyText.trim() ? "linear-gradient(135deg,#3b82f6,#2563eb)" : "#2a3a4a",
                        color: replyText.trim() ? "#fff" : "#2a3a4a",
    ...
    <div style={{
      ...
      <div style={{
        ...
        <h2 style={{
          ...
          color: "#e2e8f0", marginBottom: 24, textAlign: "center",
        }}>
        ...
        <button onClick={handleSubmit} disabled={loading}
          style={{
            ...
            boxShadow: "0 4px 16px rgba(59,130,246,0.25)", transition: "all 0.2s",
          }}
        >
        ...
        <div style={{ ...S.container, marginTop: isMobile ? 48 : 0 }}>
          ...
          <main style={S.feed}>
            ...
            {activePage === "home" && (
              <>
                <div style={S.feedHeader}>
                  <div style={{ padding: "16px 20px 0", fontFamily: "'DM Serif Display', serif", fontSize: 20, color: "#e2e8f0" }}>Hjem</div>
                ...
                {/* Compose */}
                <div style={{ padding: "16px 20px", borderBottom: "1px solid #2a3a4a", display: "flex", gap: 12 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <textarea
                      ...
                      style={{
                        width: "100%", background: "transparent", border: "none", outline: "none",
                        resize: "none", color: "#e2e8f0", fontSize: 18, minHeight: 70,
                    ...
                    {/* Poll-opprettar */}
                    {showPollCreator && (
                      <div style={{ marginBottom: 10, padding: 12, background: "#1a2535", borderRadius: 12, border: "1px solid #2a3a4a" }}>
                        {pollOptions.map((opt, i) => (
                          <input key={i} value={opt} placeholder={`Alternativ ${i + 1}`}
                            onChange={e => setPollOptions(prev => prev.map((o, j) => j === i ? e.target.value : o))}
                            style={{
                              width: "100%", background: "#152030", border: "1px solid #2a3a4a",
                        ...
                        <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                          {pollOptions.length < 4 && (
                            <button onClick={() => setPollOptions(prev => [...prev, ""])}
                              style={{ background: "none", border: "1px solid #2a3a4a", borderRadius: 8, padding: "4px 12px", color: "#3b82f6", cursor: "pointer", fontSize: 13 }}>
                              + Alternativ
                            </button>
                          )}
                          <select value={pollDuration} onChange={e => setPollDuration(parseInt(e.target.value))}
                            style={{ background: "#152030", border: "1px solid #2a3a4a", borderRadius: 8, padding: "4px 8px", color: "#e2e8f0", fontSize: 13 }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: "1px solid #2a3a4a", paddingTop: 10 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        {charCount > 200 && (
                          <span style={{ color: charCount > 260 ? "#f91880" : "#94a3b8", fontSize: 13 }}>
                            {280 - charCount}
                          </span>
                        )}
                        <button onClick={handlePost}
                          ...
                          style={{
                            background: newPost.trim() && !posting ? "linear-gradient(135deg,#3b82f6,#2563eb)" : "#2a3a4a",
                            color: newPost.trim() && !posting ? "#fff" : "#2a3a4a",
            ...
            {activePage === "explore" && (
              <>
                <div style={S.feedHeader}>
                  <div style={{ padding: "14px 20px", fontFamily: "'DM Serif Display', serif", fontSize: 20, color: "#e2e8f0" }}>Utforsk</div>
                  <div style={{ padding: "0 20px 14px" }}>
                    <div style={{ background: "#152030", borderRadius: 24, padding: "10px 16px", display: "flex", gap: 10, border: "1px solid #2a3a4a" }}>
                      <span style={{ color: "#3b82f6" }}>🔍</span>
                      <input value={searchQuery}
                        onChange={e => handleSearch(e.target.value)}
                        placeholder="Søk innlegg og brukere"
                        style={{ background: "none", border: "none", outline: "none", color: "#e2e8f0", flex: 1, fontSize: 15, fontFamily: "'Crimson Pro', serif" }}
                      />
                    </div>
                  </div>
                </div>
                  ...
                  <>
                    {searchResults.users.length > 0 && (
                      <div style={{ borderBottom: "1px solid #2a3a4a" }}>
                        <div style={{ padding: "12px 20px", color: "#94a3b8", fontSize: 13, fontWeight: 700 }}>BRUKERE</div>
                        {searchResults.users.map(u => (
                          ...
                          >
                            ...
                            <div>
                              <div style={{ fontWeight: 700, color: "#e2e8f0", fontSize: 15 }}>{u.name} {u.verified && <span style={{ color: "#3b82f6" }}>✓</span>}</div>
                              <div style={{ color: "#94a3b8", fontSize: 13 }}>@{u.handle} · {formatCount(u.followers_count)} følgere</div>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    {searchResults.posts.length > 0 && (
                      <>
                        <div style={{ padding: "12px 20px", color: "#94a3b8", fontSize: 13, fontWeight: 700 }}>INNLEGG</div>
                    ...
                    {searchResults.users.length === 0 && searchResults.posts.length === 0 && (
                      <div style={{ padding: 40, textAlign: "center", color: "#94a3b8" }}>Ingen resultater</div>
                    )}
                  </>
                ) : (
                  loadingPosts ? <Spinner /> : posts.map(post => (
            ...
            {activePage === "bookmarks" && (
              <>
                <div style={S.feedHeader}>
                  <div style={{ padding: "16px 20px", fontFamily: "'DM Serif Display', serif", fontSize: 20, color: "#e2e8f0" }}>Bokmerker</div>
                </div>
                {loadingPosts ? <Spinner /> : posts.length === 0
                  ? <div style={{ padding: 40, textAlign: "center", color: "#94a3b8" }}>Ingen bokmerker ennå</div>
                  : posts.map(post => (
                    <PostCard key={post.id} post={post} {...postCardProps} isOwn={post.user_id === currentUser.id} />
                  ))
                }
              ...
              <>
                <div style={S.feedHeader}>
                  <div style={{ padding: "14px 20px", display: "flex", alignItems: "center", gap: 12 }}>
                    <div>
                      <div style={{ fontFamily: "'DM Serif Display', serif", fontSize: 18, color: "#e2e8f0" }}>{currentUser.name}</div>
                      <div style={{ color: "#94a3b8", fontSize: 13 }}>{posts.length} innlegg</div>
                    </div>
                  </div>
                </div>
                    ...
                    <div style={{ position: "relative" }}>
                      ...
                      <button onClick={() => avatarInputRef.current?.click()} title="Endre profilbilde"
                        style={{
                          ...
                          background: "#1a2535", border: "2px solid #2a3a4a", borderRadius: "50%",
                ...
                <div style={{ padding: "0 20px 16px", borderBottom: "1px solid #2a3a4a" }}>
                  <div style={{ fontFamily: "'DM Serif Display', serif", fontSize: 20, color: "#e2e8f0" }}>
                    {currentUser.name} {currentUser.verified && <span style={{ color: "#3b82f6" }}>✓</span>}
                  </div>
                  <div style={{ color: "#94a3b8", marginBottom: 8 }}>@{currentUser.handle}</div>
                  {currentUser.bio && <p style={{ color: "#d4e8d4", fontSize: 15, marginBottom: 12 }}>{currentUser.bio}</p>}
                  <div style={{ display: "flex", gap: 20, fontSize: 14 }}>
                    <span><strong style={{ color: "#e2e8f0" }}>{currentUser.following_count ?? 0}</strong> <span style={{ color: "#94a3b8" }}>Følger</span></span>
                    <span><strong style={{ color: "#e2e8f0" }}>{currentUser.followers_count ?? 0}</strong> <span style={{ color: "#94a3b8" }}>Følgere</span></span>
                  </div>
                </div>
                {loadingPosts ? <Spinner /> : posts.map(post => (
              ...
              <>
                <div style={S.feedHeader}>
                  <div style={{ padding: "14px 20px", display: "flex", alignItems: "center" }}>
                    ...
                    <div>
                      <div style={{ fontFamily: "'DM Serif Display', serif", fontSize: 18, color: "#e2e8f0" }}>{viewProfile?.name || "..."}</div>
                      <div style={{ color: "#94a3b8", fontSize: 13 }}>{viewProfilePosts.length} innlegg</div>
                    </div>
                  </div>
                </div>
                {viewProfile ? (
                  <>
                    <div style={{ height: 120, background: "linear-gradient(135deg,#0a2a0a,#3b82f633)", position: "relative", marginBottom: 56 }}>
                      ...
                      <div style={{ position: "absolute", bottom: -40, right: 20, display: "flex", gap: 8 }}>
                        <button onClick={() => startConversation(viewProfile)}
                          style={{ background: "transparent", border: "1px solid #94a3b8", borderRadius: 20, padding: "6px 14px", color: "#e2e8f0", cursor: "pointer", fontSize: 13, fontFamily: "'DM Serif Display', serif" }}>
                          ✉️
                        </button>
                        <button onClick={() => handleFollow(viewProfile.handle)}
                          style={{
                            background: viewProfile.is_following ? "transparent" : "#e2e8f0",
                            color: viewProfile.is_following ? "#e2e8f0" : "#0f1923",
                            border: viewProfile.is_following ? "1px solid #94a3b8" : "none",
                    ...
                    <div style={{ padding: "0 20px 16px", borderBottom: "1px solid #2a3a4a" }}>
                      ...
                      <div style={{ color: "#94a3b8", marginBottom: 8 }}>@{viewProfile.handle}</div>
                      {viewProfile.bio && <p style={{ color: "#d4e8d4", fontSize: 15, marginBottom: 12 }}>{viewProfile.bio}</p>}
                      <div style={{ display: "flex", gap: 20, fontSize: 14 }}>
                        <span><strong style={{ color: "#e2e8f0" }}>{viewProfile.following_count ?? 0}</strong> <span style={{ color: "#94a3b8" }}>Følger</span></span>
                        <span><strong style={{ color: "#e2e8f0" }}>{viewProfile.followers_count ?? 0}</strong> <span style={{ color: "#94a3b8" }}>Følgere</span></span>
                      </div>
                    </div>
            ...
            {activePage === "notifications" && (
              <>
                <div style={S.feedHeader}>
                  <div style={{ padding: "16px 20px", fontFamily: "'DM Serif Display', serif", fontSize: 20, color: "#e2e8f0" }}>Varsler</div>
                </div>
                {loadingNotifs ? <Spinner /> : notifications.length === 0
                  ? <div style={{ padding: 40, textAlign: "center", color: "#94a3b8" }}>Ingen varsler ennå</div>
                  : notifications.map(n => (
                    <div key={n.id} onClick={() => n.post_id && openThread(n.post_id)}
                      style={{
                        display: "flex", gap: 12, padding: "16px 20px", borderBottom: "1px solid #2a3a4a",
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
                          ...
                          <span style={{ fontWeight: 700, color: "#e2e8f0", fontSize: 14 }}>{n.from_name}</span>
                        ...
                        {n.post_content && (
                          <p style={{ color: "#94a3b8", fontSize: 13, marginTop: 4, fontStyle: "italic" }}>
                            "{n.post_content.slice(0, 80)}{n.post_content.length > 80 ? "..." : ""}"
                          </p>
                        )}
                        <span style={{ color: "#94a3b8", fontSize: 12 }}>{timeAgo(n.created_at)}</span>
                      </div>
                    </div>
                  ))
                }
              ...
              <>
                <div style={S.feedHeader}>
                  <div style={{ padding: "16px 20px", display: "flex", alignItems: "center" }}>
                    ...
                    <span style={{ fontFamily: "'DM Serif Display', serif", fontSize: 20, color: "#e2e8f0" }}>
                ...
                {!activeConversation ? (
                  ...
                  loadingMessages ? <Spinner /> : conversations.length === 0
                    ? <div style={{ padding: 40, textAlign: "center", color: "#94a3b8" }}>
                        ...
                        <div style={{ fontFamily: "'DM Serif Display', serif", fontSize: 22, color: "#e2e8f0", marginBottom: 8 }}>Ingen meldinger ennå</div>
                    ...
                    : conversations.map(c => (
                      <div key={c.other_user_id} onClick={() => setActiveConversation(c)}
                        style={{
                          ...
                          padding: "14px 20px", borderBottom: "1px solid #2a3a4a", cursor: "pointer",
                        }}
                        onMouseEnter={e => e.currentTarget.style.background = "rgba(59,130,246,0.05)"}
                        onMouseLeave={e => e.currentTarget.style.background = "transparent"}
                      >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: "flex", justifyContent: "space-between" }}>
                            ...
                            <span style={{ color: "#94a3b8", fontSize: 12 }}>{timeAgo(c.last_message_at)}</span>
                          </div>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            <p style={{ color: "#94a3b8", fontSize: 13, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 300 }}>
                              {c.last_sender_id === currentUser.id && "Du: "}{c.last_message}
                            </p>
                            {c.unread_count > 0 && (
                              <span style={{ background: "#3b82f6", color: "#0f1923", borderRadius: 10, padding: "1px 7px", fontSize: 11, fontWeight: 700, flexShrink: 0 }}>
                                {c.unread_count}
                              </span>
                            )}
                          </div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 60px)" }}>
                      ...
                      {loadingMessages ? <Spinner /> : chatMessages.map(msg => (
                        <div key={msg.id} style={{
                          ...
                          <div style={{
                            ...
                            background: msg.sender_id === currentUser.id ? "#3b82f6" : "#152030",
                            border: msg.sender_id === currentUser.id ? "none" : "1px solid #2a3a4a",
                          }}>
                            <p style={{ color: "#e2e8f0", fontSize: 14, lineHeight: 1.5 }}>{msg.content}</p>
                            <span style={{ color: msg.sender_id === currentUser.id ? "#60a5fa" : "#94a3b8", fontSize: 11 }}>
                    ...
                    <div style={{ padding: "12px 20px", borderTop: "1px solid #2a3a4a", display: "flex", gap: 10 }}>
                      <input value={messageText}
                        onChange={e => setMessageText(e.target.value)}
                        onKeyDown={e => e.key === "Enter" && handleSendMessage()}
                        placeholder="Skriv en melding..."
                        style={{
                          flex: 1, background: "#152030", border: "1px solid #2a3a4a",
                          borderRadius: 24, padding: "10px 16px", color: "#e2e8f0",
                          fontSize: 14, fontFamily: "'Crimson Pro', serif", outline: "none",
                        }}
                      />
                      <button onClick={handleSendMessage} disabled={!messageText.trim()}
                        style={{
                          background: messageText.trim() ? "linear-gradient(135deg,#3b82f6,#2563eb)" : "#2a3a4a",
                          color: messageText.trim() ? "#fff" : "#2a3a4a",
              ...
              <>
                <div style={S.feedHeader}>
                  <div style={{ padding: "16px 20px" }}>
                    <span style={{ fontFamily: "'DM Serif Display', serif", fontSize: 20, color: "#e2e8f0" }}>Admin — Innloggingslogg</span>
                  </div>
                </div>
                <div style={{ padding: "16px 20px" }}>
                  ...
                  {adminLogs.length > 0 && (
                    <div style={{ overflowX: "auto" }}>
                      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                        <thead>
                          <tr style={{ borderBottom: "2px solid #2a3a4a", textAlign: "left" }}>
                        ...
                        <tbody>
                          {adminLogs.map(log => (
                            <tr key={log.id} style={{ borderBottom: "1px solid #2a3a4a" }}>
                              <td style={{ padding: "10px 8px", color: "#e2e8f0" }}>
                                <div style={{ fontWeight: 700 }}>{log.name}</div>
                                <div style={{ color: "#94a3b8", fontSize: 11 }}>@{log.handle}</div>
                              </td>
                              <td style={{ padding: "10px 8px", color: "#94a3b8", fontFamily: "monospace", fontSize: 12 }}>{log.ip_address || "—"}</td>
                              <td style={{ padding: "10px 8px", color: "#94a3b8" }}>{log.device || "—"}</td>
                              <td style={{ padding: "10px 8px", color: "#94a3b8", fontSize: 12 }}>{new Date(log.created_at).toLocaleString("no-NO")}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
            ...
            {activePage === "thread" && (
              <>
                <div style={S.feedHeader}>
                  <div style={{ padding: "14px 20px", display: "flex", alignItems: "center" }}>
                    <button onClick={() => setActivePage("home")} style={S.backBtn}>←</button>
                    <span style={{ fontFamily: "'DM Serif Display', serif", fontSize: 20, color: "#e2e8f0" }}>Innlegg</span>
                  </div>
                </div>
                {loadingThread ? <Spinner /> : threadPost && (
                  <>
                    ...
                    {threadReplies.length > 0 && (
                      <>
                        <div style={{ padding: "12px 20px", borderBottom: "1px solid #2a3a4a", color: "#94a3b8", fontSize: 13, fontWeight: 700 }}>
                          SVAR ({threadReplies.length})
                        </div>
                        {threadReplies.map(r => (
                          <div key={r.id} style={{ display: "flex", gap: 12, padding: "12px 20px", borderBottom: "1px solid #2a3a4a" }}>
                            ...
                            <div style={{ flex: 1 }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                <span style={{ fontWeight: 700, color: "#e2e8f0", fontSize: 14, cursor: "pointer" }}
                                  onClick={() => handleClickUser(r.user_handle)}>{r.user_name}</span>
                                {r.user_verified && <span style={{ color: "#3b82f6", fontSize: 12 }}>✓</span>}
                                <span style={{ color: "#94a3b8", fontSize: 13 }}>@{r.user_handle}</span>
                                <span style={{ color: "#2a3a4a" }}>·</span>
                                <span style={{ color: "#94a3b8", fontSize: 13 }}>{timeAgo(r.created_at)}</span>
                              </div>
            ...
            {activePage === "hashtag" && (
              <>
                ...
                {hashtagPosts.length === 0
                  ? <div style={{ padding: 40, textAlign: "center", color: "#94a3b8" }}>Ingen innlegg med #{hashtagView}</div>
          ...
          <aside style={S.rightSidebar}>
            <div style={{ background: "#152030", borderRadius: 24, padding: "10px 16px", display: "flex", alignItems: "center", gap: 10, marginBottom: 20, border: "1px solid #2a3a4a" }}>
              <span style={{ color: "#94a3b8" }}>🔍</span>
              <input value={searchQuery}
                onChange={e => handleSearch(e.target.value)}
                placeholder="Søk på HelpTruth"
                style={{ background: "none", border: "none", outline: "none", color: "#e2e8f0", flex: 1, fontSize: 14, fontFamily: "'Crimson Pro', serif" }}
              />
            </div>

            {/* Trending */}
            <div style={{ background: "#1a2535", borderRadius: 16, border: "1px solid #2a3a4a", marginBottom: 16, overflow: "hidden" }}>
              <div style={{ padding: "14px 16px", fontFamily: "'DM Serif Display', serif", fontSize: 18, color: "#e2e8f0", borderBottom: "1px solid #2a3a4a" }}>
                Trending i Norge
              </div>
              {TRENDING.map((t, i) => (
                <div key={i} onClick={() => handleClickUser(null, t.tag.slice(1))}
                  style={{ padding: "12px 16px", borderBottom: i < TRENDING.length - 1 ? "1px solid #2a3a4a" : "none", cursor: "pointer", transition: "background 0.15s" }}
                  onMouseEnter={e => e.currentTarget.style.background = "rgba(59,130,246,0.04)"}
                  onMouseLeave={e => e.currentTarget.style.background = "transparent"}
                >
                  <div style={{ color: "#94a3b8", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.5px" }}>{t.category} · Trend</div>
                  <div style={{ color: "#3b82f6", fontWeight: 700, fontSize: 15, marginTop: 2 }}>{t.tag}</div>
                  <div style={{ color: "#94a3b8", fontSize: 12, marginTop: 2 }}>{t.posts}</div>
                </div>
              ))}
            </div>

            {/* Hvem å følge */}
            <div style={{ background: "#1a2535", borderRadius: 16, border: "1px solid #2a3a4a", overflow: "hidden", marginBottom: 16 }}>
              <div style={{ padding: "14px 16px", fontFamily: "'DM Serif Display', serif", fontSize: 18, color: "#e2e8f0", borderBottom: "1px solid #2a3a4a" }}>
                Hvem å følge
              </div>
              {[
                ...
                <div key={i} style={{ padding: "12px 16px", borderBottom: i < 2 ? "1px solid #2a3a4a" : "none", display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0, cursor: "pointer" }} onClick={() => handleClickUser(u.handle)}>
                    <div style={{ fontWeight: 700, fontSize: 14, color: "#e2e8f0", fontFamily: "'DM Serif Display', serif" }}>{u.name}</div>
                    <div style={{ color: "#94a3b8", fontSize: 12 }}>@{u.handle}</div>
                  </div>
                  <button onClick={() => handleFollow(u.handle)}
                    style={{
                      background: following[`@${u.handle}`] ? "transparent" : "#e2e8f0",
                      color: following[`@${u.handle}`] ? "#e2e8f0" : "#0f1923",
                      border: following[`@${u.handle}`] ? "1px solid #94a3b8" : "none",
            ...
            <div style={{ color: "#2a3a4a", fontSize: 11, lineHeight: 2 }}>
