...
function AuthPage({ onLogin, googleError = "", googleContext = {} }) {
  const tenant = useTenant();
  const hasNeedInvite = googleError === "need_invite";
  const [mode, setMode]         = useState(hasNeedInvite ? "register" : "login");
  const [form, setForm]         = useState({
    name:        googleContext.name  || "",
    handle:      "",
    email:       googleContext.email || "",
    password:    "",
    invite_code: "",
  });
  ...
  const googleErrorMsg = (() => {
    ...
    switch (googleError) {
      case "need_invite":
        return isBreedz
          ? "No BREEDZ account found for this Google email. Enter an invitation code below, then click \"Continue with Google\" again."
          : "Ingen konto funnet for denne Google-eposten. Fyll inn invitasjonskoden nedenfor og klikk \"Continue with Google\" på nytt.";
      case "invalid_invite":
        return isBreedz ? "That invitation code is not valid." : "Invitasjonskoden er ikke gyldig.";
      case "email_not_verified":
        return isBreedz ? "Your Google email is not verified." : "Google-eposten din er ikke verifisert.";
      case "oauth_denied":
        return isBreedz ? "Google sign-in was cancelled." : "Google-innlogging ble avbrutt.";
      default:
        return isBreedz ? "Google sign-in failed. Please try again." : "Google-innlogging feilet. Prøv igjen.";
    }
  })();
        ...
        <div className="glass-card glow-border" style={{ padding: 32 }}>
          {googleErrorMsg && (
            <div style={{
              ...
              color: "var(--danger)", fontSize: 14, lineHeight: 1.45,
            }}>
              {googleErrorMsg}
            </div>
          )}
          ...
          <button
            ...
            onClick={() => {
              ...
              window.location.href = `${BASE_URL}/auth/google?${params.toString()}`;
            }}
            style={{
              width: "100%",
          ...
          >
            ...
            {isBreedz ? "Continue with Google" : "Fortsett med Google"}
          </button>
...
export default function HelpTruth() {
  ...
  // Google OAuth retur-state (plukket opp fra URL ved redirect tilbake fra backend)
  const [googleError, setGoogleError] = useState("");
  const [googleContext, setGoogleContext] = useState({ email: "", name: "" });

  // ── Auth check + Google OAuth callback ──────────────────────────────────
  useEffect(() => {
    // Plukk opp Google-retur-parametre
    const url = new URL(window.location.href);
    const sp = url.searchParams;
    const tokenParam  = sp.get("token");
    const provider    = sp.get("provider");
    const linked      = sp.get("linked");
    const googleErr   = sp.get("google_error");
    const googleEmail = sp.get("google_email");
    const googleName  = sp.get("google_name");

    // Rydd URL så refresh ikke re-prosesserer
    if (tokenParam || googleErr || linked) {
      ["token","provider","linked","google_error","google_email","google_name"].forEach(k => sp.delete(k));
      const cleaned = url.pathname + (sp.toString() ? "?" + sp.toString() : "");
      window.history.replaceState({}, "", cleaned);
    }

    if (tokenParam && provider === "google") {
    ...
    if (googleErr) {
      ...
      if (googleEmail || googleName) {
        setGoogleContext({ email: googleEmail || "", name: googleName || "" });
      }
    }
    ...
    if (token) {
      api("/auth/me")
        .then(user => {
          setCurrentUser(user);
          if (linked === "1") {
            setTimeout(() => showToast("Google-kontoen din er koblet til din eksisterende konto."), 100);
          }
        })
        .catch(() => removeToken())
  ...
  if (!currentUser) return <AuthPage
    onLogin={user => { setCurrentUser(user); setAuthChecked(true); }}
